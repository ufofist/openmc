...
def get_openmc_universes(cells, surfaces, materials):
    ...
    for c in cells:
        ...
        if 'fill' in c['parameters']:
            ...
            cell.fill = materials[c['material']]
            if cell.fill.density is None:
                if c['density'] > 0:
                    cell.fill.set_density('atom/b-cm', c['density'])
                else:
                    cell.fill.set_density('g/cm3', abs(c['density']))
            else:
                if cell.fill.density != abs(c['density']):
                    print("WARNING: Cell {} has same material but with a "
                          "different density than that of another.".format(c['id']))
                    copy_mat = copy.deepcopy(cell.fill)
                    copy_mat.id = None
                    cell.fill = copy_mat
                    if c['density'] > 0:
                        cell.fill.set_density('atom/b-cm', c['density'])
